...
# Config
st.set_page_config(page_title="Gemini+Groq Multi-file Chatbot", page_icon="🤖", layout="wide")
st.title("🤖 Multi-backend Multi-file Chatbot — Gemini / Groq")
st.markdown(
    "Unggah file (PDF / TXT / DOCX / PPTX). Bangun FAISS vector store dari dokumen, lalu pilih backend LLM untuk menjawab "
    "(Google Gemini atau Groq Cloud)."
)
...
# UI: sidebar
# -------------------------
st.sidebar.header("📂 Unggah & Bangun")
uploaded_files = st.sidebar.file_uploader(
    "Unggah file (pdf, txt, docx, pptx) — boleh banyak",
    type=["pdf", "txt", "docx", "pptx"],
    accept_multiple_files=True
)
build_btn = st.sidebar.button("🚀 Bangun Vector Store")
clear_btn = st.sidebar.button("🧹 Reset vector store")

# backend choices
st.sidebar.markdown("### Pilih backend LLM")
backend = st.sidebar.radio("Backend LLM", options=["google_gemini", "groq"])
...
# Query
# -------------------------
prompt = st.text_input("Tanyakan sesuatu berdasarkan dokumen yang diunggah:", placeholder="Misal: Ringkas dokumen tentang topik X...")
        ...
        try:
            if backend == "google_gemini":
                ...
                if not GROQ_AVAILABLE:
                    st.error("❌ Groq SDK tidak terinstal. Instal groq atau langchain-groq dan restart.")
