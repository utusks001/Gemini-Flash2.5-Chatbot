...
# ------------------------------------------------------------------
st.set_page_config(
    page_title="Gemini+Groq Multi-file Chatbot", page_icon="🤖", layout="wide"
)
st.title("🤖 Multi-backend Multi-file Chatbot — Gemini / Groq")

st.markdown(
    "Upload file (PDF / TXT / DOCX / PPTX). Build FAISS vector store from documents, then "
    "pick the LLM backend to answer (Google Gemini or Groq Cloud)."
)
...
GROQ_API_KEY = (
    ...
    or os.getenv("GROQ_SECRET_ACCESS_KEY")
...
def extract_text_from_file(uploaded_file) -> str:
    ...
    if name.endswith(".pdf"):
        ...
        st.warning(
            f"⚠️ File {uploaded_file.name} berformat lama (.doc/.ppt). Silakan konversi ke .docx/.pptx."
        )
        return ""
    else:
...
backend = st.sidebar.radio(
    "Backend LLM", options=["google_gemini", "groq"]
)
...
st.sidebar.write(
    f"- Google API key present: {'✅' if GOOGLE_API_KEY else '❌'}"
)
        ...
        try:
            if backend == "google_gemini":
                if not GOOGLE_API_KEY:
                    st.error(
                        "❌ GOOGLE_API_KEY tidak ditemukan. Set environment variable atau isi di .env."
                    )
                else:
                ...
                if not GROQ_AVAILABLE:
                    st.error(
                        "❌ Groq SDK tidak terinstal. Install groq atau langchain-groq dan restart."
                    )
                elif not GROQ_API_KEY:
                    st.error(
                        "❌ GROQ API key tidak ditemukan. Set environment variable GROQ_API_KEY or GROQ_SECRET_ACCESS_KEY."
                    )
                else:
                    # direct minimal usage of groq Cloud SDK
                    with st.spinner("🤖 Menghubungi Groq Cloud..."):
                        model_name = st.sidebar.text_input(
                            "Groq model (contoh: llama2-13b-2048)", value="llama2-13b-2048"
                        )
